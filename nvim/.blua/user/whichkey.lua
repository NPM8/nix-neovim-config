...
local setup = {
  ...
  icons = {
    ...
    separator = "Γ₧£", -- symbol used between a key and it's label
...
local mappings = {
  ...
  g = {
    ...
    d = {
      name= "Diff",
      h = { "<cmd>lua vim.cmd.DiffviewFileHistory(vim.api.nvim_buf_get_name(0))<cr>", "[diffview] file history (current buffer)"},
      d = {"<cmd>lua vim.cmd.DiffviewOpen<cr>", "[diffview] open" },
      f = {"<cmd>lua vim.cmd.DiffviewToggleFiles<cr>", "[diffview] toggle files" },
      g = {"<cmd>lua vim.cmd.DiffviewFileHistory<cr>", "[diffview] file history (cwd)" },
    },
    --[[
    TODO: check this github and make it usefull in future
    h = {
        name = "+Github",
        c = {
            name = "+Commits",
